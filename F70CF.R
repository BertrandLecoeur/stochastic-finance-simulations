################################################################
###  Methods for F70CF  ########################################
###    - implicit and explicit Finite Difference Methods  ######
...
"implicit_FD_one_step" <- function(pt, p0, r, sigma, deltaT){
  ...
  # pt are the prices at time t+deltaA (end of subperiod)
  # p0 are prices at time t (start of subperiod), 
  #    only p0[1] and p0[M+1] are used: 
  #       - p0[1] is the price for the highest possible share price
  #       - p0[M+1] is the option price for S_t = 0
  # r is the interest rate p.a.
  # sigma is the volatility p.a.
  # deltaT is the length of the time step, length of a subperiod
  # the function returns p0, the option prices at time t, the start of 
  #    the subperiod [t, t+deltaT]
  M = length(pt)-1  
...
"FD_Euro" <- function(p, r, sigma, deltaT, explicit = FALSE){
  ...
  # NOTE: p[,1] is for St = St_max, p[,M+1] is for St = 0
  ...
  for (t in (N:1)){ # start with last column (time = N * deltaT)
    if (explicit)
      p[,t] = explicit_FD_one_step(p[,t+1], p[,t], r, sigma, deltaT)
    else
